...
import enviNFe_v400.TNFe.InfNFe.Det.Imposto.ICMS.ICMS60;
...
public class Icms60 implements IImposto {
    ...
    public void verificaImposto(DetalhamentoProdutoServico item, ICMS icms) throws NFeException {
        if (item.getCsticms().equals(NFeTipoSituacaoTributariaICMS.COD60)) {
            ...
            icms60.setVBCFCPSTRet(item.getVbcfcpst() != null ? item.getVbcfcpst().toString() : null);
            icms60.setPFCPSTRet(item.getPfcpstret() != null ? item.getPfcpstret().toString() : null);
            icms60.setVFCPSTRet(item.getVfcpstret() != null ? item.getVfcpstret().toString() : null);
            icms60.setPST(item.getPst() != null ? item.getPst().toString() : null);
